...
<section id="map-section" class="py-24 bg-paper relative border-t border-ink/10 overflow-hidden">
    ...
    <div class="map-shell w-full h-[750px] relative z-10 shadow-2xl overflow-hidden" id="main-map-shell">
        <div class="w-full h-full" id="main-map"></div>
        <div class="map-lock-overlay" id="main-map-overlay">
            <div class="map-lock-overlay__panel">
                <p class="map-lock-overlay__eyebrow">Esperienza controllata</p>
                <h3 class="map-lock-overlay__title">La mappa resta ferma durante lo scroll</h3>
                <p class="map-lock-overlay__text">Attivala solo quando vuoi esplorare posizioni e dettagli. Finché non la attivi, la pagina continua a scorrere senza interferenze.</p>
                <button type="button" class="map-lock-overlay__button" id="main-map-toggle" aria-pressed="false">
                    Attiva la mappa
                </button>
            </div>
        </div>
        <?php if ( empty( $map_properties ) ) : ?>
            <div class="map-empty-state" id="main-map-empty-state">
                <span class="map-empty-state__eyebrow">In preparazione</span>
                <p class="map-empty-state__title">Nessun immobile geolocalizzato in questo momento</p>
                <p class="map-empty-state__text">Quando completiamo indirizzi e coordinate nel gestionale, qui compariranno i marker reali della home.</p>
            </div>
        <?php endif; ?>
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    if (typeof L !== 'undefined') {
        ...
        const mapShell = document.getElementById('main-map-shell');
        ...
        mapToggle.addEventListener('click', function() {
            const shouldEnable = mapToggle.getAttribute('aria-pressed') !== 'true';
            setMapInteractive(shouldEnable);
            if (shouldEnable) {
